...
疫情爆发的最初几周，疫苗必须在大规模、异质的人群中分配，每日剂量预算严格受限，决策周期跨越数周到数月。任何可部署的策略最终都必须**指名具体的个体**——而不是给出群体层面的比例——并且要同时应对三大结构性难题：长时序的延迟奖励、$\binom{N}{K}$ 量级的组合动作空间、以及节点在网络中的位置和它的人口学群体身份同样重要。
